"""Primary generator: Stable Diffusion 3.5 + ControlNet (CLAUDE.md §3, Stage B/C).
...
Two phases:
  * finetune() — prepares the on-disk data needed for the heavy training and
    ...
    ControlNet). We don't reimplement those trainers; we hand them prepared data.
    See docs/synthetic_data_guide.md for the cluster walkthrough.
...
class SD35ControlNet(Generator):
    ...
    def __init__(self, config: dict | None = None):
        ...
        # Layout-mask conditioning -> free labels (CLAUDE.md §3). 'seg' is the
    ...
    def finetune(self, train_yaml: Path) -> None:
        ...
        We don't run the trainers here (they're the diffusers examples, and they
        belong on the cluster). We export what they need and tell the user how to
        launch them. See docs/synthetic_data_guide.md.
        """
        ...
        if self.control_kind == "seg":
            ...
            print("  Train a seg ControlNet with diffusers examples/controlnet/"
                  "train_controlnet_sd3.py (see docs/synthetic_data_guide.md).")
        else:
